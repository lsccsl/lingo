	...
	for _, pkg := range pkgs {
		...
		for keyFileName, file := range pkg.Files {
			...
			for _, valobj := range file.Scope.Objects {
				...
				if !b {
					common.LogDebug("not msg type name:",valobj.Name)
					continue
				}
				common.LogDebug("obj name:",valobj.Name, " name:", msgName, " type:", msgType)
...
func ProtoParseAdd(name string, msgTye int32){
	...
	if ok{
		common.LogErr("msg type all ready exist:", msgTye)
		return
	}
...
func ProtoParseByName(binMsg []byte, msgType int32)proto.Message {
	...
	if !ok{
		common.LogDebug("can't find msg type:", msgType)
		return nil
	}
...
func ParseDump() {
	for k, v := range mapProtoMsgParse {
		common.LogDebug("type:", k, " name:", v.fullName)
	}
}