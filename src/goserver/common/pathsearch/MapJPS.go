...
func (pthis*JSPMgr)addNode(node *JPSNode, parent *JPSNode) {
	...
	if pthis.isInHistory(node.pos) {
		common.LogErr("already in history")
		return
	}
...
func (pthis*MapData)DumpNodeSub(searchMgr *JSPMgr, node *JPSNode, bmp *common.Bitmap) {
	tmpBMP := bmp.BmpData
...
func (pthis*MapData)DumpJPSMap(strMapFile string, path []Coord2d, searchMgr *JSPMgr) {
	...
	widBytePitch := common.CalWidBytePitch(pthis.widReal, 24)
	dataLen := widBytePitch * pthis.hei
	tmpBMP := make([]uint8, dataLen)
	bmp := common.CreateBMP(pthis.widReal, pthis.hei, 24, nil)
	bmp.BmpData = tmpBMP
