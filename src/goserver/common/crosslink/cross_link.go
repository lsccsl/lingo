...
func (pthis*Crosslink_mgr)Crosslink_mgr_add(node_param *Crosslink_node_param) int {
	...
	if ok {
		common.LogErr("already has node id:", new_node_id)
		return 0
	}
...
func(pthis*Crosslink_mgr)_inter_crosslink_link_guard(new_node *cross_node) {
	...
	if pthis.link_x_._inter_crosslink_empty() {
		common.LogErr("err, x link head is nil")
	}
	if pthis.link_y_._inter_crosslink_empty() {
		common.LogErr("err, y link head is nil")
	}
...
func (pthis*Crosslink_mgr)Crosslink_mgr_update_pos(node_id int, coord_x float32, coord_y float32) {
	...
	x_map := make(map[int]int)
	{
		...
		if coord_x < old_x {
			...
			if x_cur_node == nil {
				common.LogDebug("link x head", node_id)
			...
			if x_cur_node == nil {
				common.LogDebug("link x tail", node_id)
...

	{
		...
		if coord_y < old_y {
			...
			if y_cur_node == nil {
				common.LogDebug("link y head", node_id)
			...
			if y_cur_node == nil {
				common.LogDebug("link y tail", node_id)
...
func (pthis*Crosslink_mgr)Check() {
	...
	// check x y coord sort is right
	{
		...
		for ; cur_node != nil; {
			...
			if node_next != nil {
				if node_next.get_coord() < cur_node.get_coord() {
					common.LogErr("wrong coord sort", cur_node, node_next)
			...
			if cur_node.get_node_type() == CROSSLINK_NODE_TYPE_node {
				...
				if !ok {
					common.LogErr("x node is not is map", cur_node)
					panic("cross link check err")
				}
		...
		if len(map_node_id) != 0 {
			common.LogErr("node is not match")
			panic("cross link check err")
		}
		...
		for ; cur_node != nil; {
			...
			if cur_node.get_node_type() == CROSSLINK_NODE_TYPE_node {
				_, ok := map_node_id[cur_node.get_node_id()]
				if !ok {
					common.LogErr("y node is not is map", cur_node)
					panic("cross link check err")
				}
				delete(map_node_id, cur_node.get_node_id())
			}
	...
	for k, v := range pthis.map_node_ {
		...
		//check x front guard
		{
			...
			if b_find_front == false {
				common.LogErr("can't find x front guard", cur_node)
				panic("cross link check err")
				break
			}
		}
		...
		//check x back guard
		{
			...
			if b_find_back == false {
				common.LogErr("can't find x back guard", cur_node)
				panic("cross link check err")
				break
			}
		}
		...
		// check y front guard
		{
			...
			if b_find_front == false {
				common.LogErr("can't find y front guard", cur_node)
				panic("cross link check err")
				break
			}
		}
		...
		// check y back guard
		{
			...
			if b_find_back == false {
				common.LogErr("can't find y back guard", cur_node)
	...
	for k, v := range pthis.map_node_ {
		...
		for k, _ := range map_view_by_x {
			...
			if ok {
				_, ok1 := map_view_by[k]
				if !ok1 {
					common.LogErr("node view by err:", v)
					panic("cross link check err")
					break
				}
