...
func (pthis*TcpSrvMgrUnit)process_srvEvt_addremote(evt *srvEvt_addremote){
	...
	if oldSrv == nil {
		common.LogErr(" fail add srv:", evt.srvID, " addr:", evt.addr)
		return
	}
	...
	if bDial {
		...
		if err != nil {
			common.LogErr("connect to srv:", evt.srvID, " dial err")
		}
		common.LogDebug("srv:", evt.srvID, " fd:", oldSrv.fdDial.String(), " addr:", evt.addr)
	}
}
...
func (pthis*TcpSrvMgrUnit)process_srvEvt_DialSuc(evt *srvEvt_TcpDialSuc){
	oldSrv := pthis.getSrv(evt.srvID)
	if oldSrv == nil {
		common.LogDebug(" dial suc, no srv:", evt.srvID, " dial suc, fd:", evt.fdDial.String())
		return
	}
	common.LogDebug("srv:", evt.srvID, " dial suc, fd:", evt.fdDial.String())
	oldFD := oldSrv.fdDial
...
func (pthis*TcpSrvMgrUnit)process_srvEvt_SrvReport(evt *srvEvt_SrvReport) {
	common.LogDebug("srv:", evt.srvID, " acpt fd:", evt.fdAcpt.String())
	oldSrv := pthis.getSrv(evt.srvID)
	if oldSrv == nil {
		common.LogDebug("no srv:", evt.srvID, " acpt fd:", evt.fdAcpt.String())
		return
	}
	oldFD := oldSrv.fdAcpt
...
func (pthis*TcpSrvMgrUnit)process_srvEvt_TcpClose(evt *srvEvt_TcpClose){
	...
	if oldSrv.fdDial.IsSame(&evt.fd) && !oldSrv.fdDial.IsNull() {
		oldSrv.fdDial = common.FD_DEF_NIL
		common.LogDebug(" dail srv:", evt.srvID, " fd:", evt.fd.String())
		return
...
/*		fd, err := pthis.tcpSrvMgr.eSrvMgr.lsn.EPollListenerDial(oldSrv.addr, &TcpAttachData{srvID : oldSrv.srvID})
	...
	if oldSrv.fdAcpt.IsSame(&evt.fd) && !oldSrv.fdAcpt.IsNull(){
		oldSrv.fdAcpt = common.FD_DEF_NIL
		common.LogDebug(" acpt srv:", evt.srvID, " fd:", evt.fd.String())
		return
	}
}
...
func (pthis*TcpSrvMgrUnit)process_srvEvt_protoMsg(evt *srvEvt_protoMsg) {
	...
	case msgpacket.MSG_TYPE__MSG_SRV_REPORT_RES:
		...
		if t != nil {
			common.LogDebug("proto msg srv:", evt.srvID, " fd:", evt.fd.String(), " msg:", evt.msg, " msgtype:", evt.msgType)
		}
	default:
...
func (pthis*TcpSrvMgrUnit)process_srvEvt_RPC_Del(evt *srvEvt_RPC_Del) {
	common.LogDebug("del rpc msg srv:", evt.srvID, " rpcUUID:", evt.rpcUUID)
	oldSrv := pthis.getSrv(evt.srvID)
	if oldSrv == nil {
		return
	}
...
func (pthis*TcpSrvMgrUnit)process_MSG_HEARTBEAT(srv *TcpSrv, fd common.FD_DEF, protoMsg *msgpacket.MSG_HEARTBEAT) {
...
func (pthis*TcpSrvMgrUnit)process_MSG_HEARTBEAT_RES(srv *TcpSrv, fd common.FD_DEF, protoMsg *msgpacket.MSG_HEARTBEAT_RES) {
	//lin_common.LogDebug(" HBRES srv:", srv.srvID, " fd", fd.String(), " from srv:", protoMsg.Id)
	srv.timestampLastHeartbeat = time.Now().Unix()
}

func (pthis*TcpSrvMgrUnit)process_MSG_RPC(srv *TcpSrv, fd common.FD_DEF, protoMsg *msgpacket.MSG_RPC) {
	pthis.totalRPCIn ++
...
func (pthis*TcpSrvMgrUnit)process_MSG_RPC_RES(srv *TcpSrv, fd common.FD_DEF, protoMsg *msgpacket.MSG_RPC_RES) {
	if srv == nil {
		return
	}
