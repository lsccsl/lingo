...
import (
	"goserver/common"
	"goserver/common/navmeshwrapper"
...
func ConstructorTcpClient(pu *TcpClientMgrUnit, fd common.FD_DEF, clientID int64) *TcpClient {
	tc := &TcpClient{
		fd :            fd,
		pu :            pu,
		clientID :      clientID,
	...
	tc.timerConnClose = time.AfterFunc(tc.durationClose,
		func(){
			tnow := time.Now().Unix()
			common.LogDebug("timeout close clientid:", tc.clientID, " fd:", tc.fd.String(),
...
func (pthis*TcpClient)Destructor() {
	common.LogDebug(" clientid:", pthis.clientID, " fd:", pthis.fd.String())
...
func (pthis*TcpClient)Process_MSG_TEST(msg *msgpacket.MSG_TEST) {
	...
	common.TcpWrite(pthis.fd, msgpacket.ProtoPacketToBin(uint16(msgpacket.MSG_TYPE__MSG_TEST_RES), msgRes))
...
func (pthis*TcpClient)Process_MSG_PATH_SEARCH(msg * msgpacket.MSG_PATH_SEARCH){
	common.LogDebug("path search", msg)
...
func (pthis*TcpClient)Process_MSG_NAV_SEARCH(msg *msgpacket.MSG_NAV_SEARCH) {
	common.LogDebug("nav search", msg)
	...
	common.LogDebug(msgSearch.path)
...
func (pthis*TcpClient)Process_MSG_NAV_ADD_OBSTACLE(msg * msgpacket.MSG_NAV_ADD_OBSTACLE) {
	common.LogDebug("add obstacle", msg)
...
func (pthis*TcpClient)Process_MSG_NAV_DEL_OBSTACLE(msg * msgpacket.MSG_NAV_DEL_OBSTACLE) {
	common.LogDebug("del obstacle", msg)
...
func (pthis*TcpClient)Process_MSG_NAV_GET_ALL_OBSTACLE(msg * msgpacket.MSG_NAV_GET_ALL_OBSTACLE){
	common.LogDebug("get obstacle", msg)
...
func (pthis*TcpClient)Process_protoMsg(msg *msgClient) {
	defer func() {
		err := recover()
		if err != nil {
			common.LogErr(" clientid:", pthis.clientID, " fd:", pthis.fd.String(), " err:", err, " msg:", msg)
		}
	}()
