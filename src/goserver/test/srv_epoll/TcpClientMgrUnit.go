...
func (pthis*TcpClientMgrUnit)Process_TcpClose(c *TcpClient, fd common.FD_DEF) {
	if c == nil {
		return
	}
	if !c.fd.IsSame(&fd) {
	...
	common.LogDebug(fd.String(), " clientid:", c.clientID, " fd:", fd.String())
...
func (pthis*TcpClientMgrUnit)Process_LOGIN(msg*msgClient){
	cliID := msg.clientID
	fd := msg.fd
	common.LogDebug("login:", fd.String(), " clientid:", cliID)
...
func (pthis*TcpClientMgrUnit)_go_Process_unit(){
	for {
		...
		if c == nil {
			common.LogErr("not process fd:", msg.fd.String(), " clientid:", msg.clientID, " msg:", msg.msg,
...
func (pthis*TcpClientMgrUnit)PushTcpLoginMsg(cliID int64, msgL *msgpacket.MSG_LOGIN, fd common.FD_DEF){
	pthis._chMsg <- &msgClient{clientID: cliID, fd:fd, msg:msgL, msgType: CLIENT_LOGIN}
}

func (pthis*TcpClientMgrUnit)PushTcpCloseMsg(cliID int64, fd common.FD_DEF){
	pthis._chMsg <- &msgClient{clientID: cliID, fd:fd, msgType: CLIENT_TCP_CLOSE}
}

func (pthis*TcpClientMgrUnit)PushProtoMsg(cliID int64, fd common.FD_DEF, msg proto.Message, attachData *TcpAttachData){
