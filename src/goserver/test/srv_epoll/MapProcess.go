...
func (pthis*MapProcess)process_msgAddAOIObject(msg *msgAddAOIObject){
	...
	common.LogDebug("add aoi ", msg.objID, " msg:", msg)
}
func (pthis*MapProcess)process_msgDelAOIObject(msg *msgDelAOIObject){
	common.LogDebug("del aoi ", msg.objID)
...
func (pthis*MapProcessMgr)addMapProcessMsg(msg interface{}, clientID int64, timeOut time.Duration) {
	...
	case <- time.After(timeOut):
		common.LogErr("time out clientID:", clientID, " msg:", msg)
	}
	close(chRes)
...
func ConstructMapProcessMgr(processCount int, eSrvMgr *ServerMgr) *MapProcessMgr {
	...
	if processCount <= 0 {
		common.LogErr("map process count is 0")
		return nil
	}
	...
	for i := 0; i < processCount; i ++ {
		common.LogDebug("load map process:", i)
		mp := ConstructMapProcess(mgr)
		common.LogDebug("load map process:", i, " done\r\n\r\n")
