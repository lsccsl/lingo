...
const(
	EN_CORPOOL_JOBTYPE_Rpc_req = common.EN_CORPOOL_JOBTYPE_user + 100
...
type ServerMgr struct {
	...
	ServerMapMgr
	tcpMgr *tcp2.TcpMgr
	httpSrv *common.HttpSrvMgr
	rpcPool *common.CorPool
	dialPool *common.CorPool
...
func (pthis*ServerMgr)CBReadProcess(tcpConn *tcp2.TcpConnection, recvBuf * bytes.Buffer) (bytesProcess int) {
...
func (pthis*ServerMgr)CBConnectAccept(tcpConn *tcp2.TcpConnection, err error) {
	if err != nil {
		common.LogErr(err)
	}
	if tcpConn == nil {
...
func (pthis*ServerMgr)CBConnectDial(tcpConn *tcp2.TcpConnection, err error) {
	if err != nil {
		common.LogErr(err)
	}
	if tcpConn == nil {
	...
	common.LogDebug(" local addr:", tcpConn.TcpGetConn().LocalAddr(), " remote addr:", tcpConn.TcpGetConn().RemoteAddr(), tcpConn.TcpConnectionID(), " srv:", tcpConn.SrvID)
...
func (pthis*ServerMgr)CBConnectClose(tcpConn *tcp2.TcpConnection, closeReason tcp2.TCP_CONNECTION_CLOSE_REASON) {
	common.LogDebug("id:", tcpConn.TcpConnectionID(),
	...
	if !tcpConn.IsAccept {
		...
		if srv != nil {
			...
			common.LogDebug("will check redial, can't find srv id:", tcpConn.SrvID)
		}
	} else {
		if tcpConn.SrvID != 0 {
			srv := pthis.getServer(tcpConn.SrvID)
...
func ConstructServerMgr(srvID int64, heartbeatIntervalSec int, rpcPoolCount int) *ServerMgr {
	...
	srvMgr.rpcPool = common.CorPoolInit(rpcPoolCount, rpcPoolCount/2, 60)
	srvMgr.dialPool = common.CorPoolInit(100, 50, 60)
...
func (pthis*ServerMgr)processClientLogin(clientID int64, tcpConn *tcp2.TcpConnection) {
	if tcpConn == nil {
		return
	}
	...
	if oldC != nil {
		if oldC.ClientGetConnectionID() != tcpConn.TcpConnectionID() {
			oldC.tcpConn.TcpConnectSetCloseReason(tcp2.TCP_CONNECTION_CLOSE_REASON_relogin)
...
func (pthis*ServerMgr)processMsg(tcpConn *tcp2.TcpConnection, msgType msgpacket.MSG_TYPE, protoMsg proto.Message) {
	if tcpConn.SrvID != 0 {
		srv := pthis.getServer(tcpConn.SrvID)
		if srv != nil {
...
func (pthis*ServerMgr)processSrvReport(tcpAccept *tcp2.TcpConnection, srvID int64){
	if tcpAccept.IsAccept {
		...
		common.LogDebug("no accept connection receive srv report, will close, from srv:", srvID, " to srv:", tcpAccept.SrvID,
			" conn:", tcpAccept.TcpConnectionID())
		tcpAccept.TcpConnectClose()
	}
}

func (pthis*ServerMgr)processRPCReq(tcpConn *tcp2.TcpConnection, msg *msgpacket.MSG_RPC) {
	...
	if tcpConn.SrvID != 0 {
		srv := pthis.getServer(tcpConn.SrvID)
		if srv != nil {
			err := pthis.rpcPool.CorPoolAddJob(&common.CorPoolJobData{
			...
			if err != nil {
				common.LogErr("put job err:", err, " srv:", tcpConn.SrvID)
			}
		} else {
			common.LogErr("can't find srv", tcpConn.SrvID)
			pthis.tcpMgr.TcpMgrCloseConn(tcpConn.TcpConnectionID())
			return
		}
	}
}
func (pthis*ServerMgr)processRPCRes(tcpConn *tcp2.TcpConnection, msgRPC *msgpacket.MSG_RPC_RES) {
...
func (pthis*ServerMgr)SendRPC_Async(srvID int64, msgType msgpacket.MSG_TYPE, protoMsg proto.Message, timeoutMilliSec int) (proto.Message, error) {
	srv := pthis.getServer(srvID)
	if srv == nil {
		return nil, common.GenErr(common.ERR_no_srv, " no srv:", srvID)
	}
...
func (pthis*ServerMgr)AddRemoteServer(srvID int64, ip string, port int, closeExpireSec int,
	...
	srv := pthis.getServer(srvID)
	if srv == nil {
		common.LogDebug("srv not exist:", srvID)
		srv = ConstructServer(pthis, srvID, pthis.heartbeatIntervalSec)
		srv.ServerSetDialData(ip, port, closeExpireSec, dialTimeoutSec, needRedial, redialCount)
	} else {
		common.LogDebug("srv already exist:", srvID)
		srv.ServerSetDialData(ip, port, closeExpireSec, dialTimeoutSec, needRedial, redialCount)
	}
}

func TcpConnectSendProtoMsg(tcpConn *tcp2.TcpConnection, msgType msgpacket.MSG_TYPE, protoMsg proto.Message) {
	...
	func(){
		...
		var noAcpt = 0
		{
			for _, val := range pthis.ServerMapMgr.mapServer {
				var connAcptID tcp2.TCP_CONNECTION_ID
				var connDialID tcp2.TCP_CONNECTION_ID
				if val.connAcpt != nil {
