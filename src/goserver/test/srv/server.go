...
func (pthis*Server) go_serverProcess() {
	defer func() {
		common.LogDebug("srv:", pthis.srvID, " exit process")
...
func (pthis*Server)processSrvReport(tcpAccept *tcp.TcpConnection){
	...
	common.LogDebug("srv:", pthis.srvID, " conn:", pthis.connAcpt.TcpConnectionID())
...
func (pthis*Server)processDailConnect(tcpDial *tcp.TcpConnection){
	...
	common.LogDebug(" srv:", pthis.srvID, " conn:", pthis.connDial.TcpConnectionID())
	pthis.srvMgr.tcpMgr.TcpDialDelDialData(pthis.srvID)
...
func (pthis*Server)processConnClose(tcpConn *tcp.TcpConnection){
	...
	common.LogDebug("srv:", pthis.srvID, " will redial", " tcpConn:", tcpConn.TcpConnectionID(), " addr:", pthis.ip, ":", pthis.port)
	pthis.connDial = nil
...
func (pthis*Server)processRedial(dialMsg *interMsgBeginRedial){
	...
	common.LogDebug("srv:", pthis.srvID, " ", dialMsg.ip, ":", dialMsg.port, " ", pthis.ip, ":", pthis.port, " bRedial:", bRedial)
...
func (pthis*Server)Go_ProcessRPC(tcpConn *tcp.TcpConnection, msg *msgpacket.MSG_RPC, msgBody proto.Message) {
	...
	if tDiff > msg.TimeoutWait {
		common.LogErr("rpc timeout, tDiff:", tDiff, " timeout wait:", msg.TimeoutWait,
...
func (pthis*Server)processRPCRes(tcpConn *tcp.TcpConnection, msg *msgpacket.MSG_RPC_RES, msgBody proto.Message) {
	...
	if rreq == nil {
		common.LogErr("fail find rpc:", msg.MsgId, " srv:", pthis.srvID)
		return
	}
	if rreq.chNtf != nil {
		rreq.chNtf <- msgBody
...
func (pthis*Server)SendRPC_Async(msgType msgpacket.MSG_TYPE, protoMsg proto.Message, timeoutMilliSec int) (proto.Message, error) {
	...
	if pthis.rpcMgr == nil{
		return nil, common.GenErr(common.ERR_sys, "no rpc mgr")
	}
	...
	if err != nil {
		common.LogErr(err)
		return nil, common.GenErr(common.ERR_sys, "packet err")
	}
	...
	case <-time.After(time.Millisecond * time.Duration(timeoutMilliSec)):
		...
		err = common.GenErr(common.ERR_rpc_timeout, " rpc time out srv:", pthis.srvID, " rpcid:", msgRPC.MsgId)
	}
	...
	if res == nil {
		return nil, common.GenErr(common.ERR_sys, "msg is nil")
	}
...
func (pthis*Server) process_MSG_HEARTBEAT (tcpConn *tcp.TcpConnection, protoMsg * msgpacket.MSG_HEARTBEAT) {
	common.LogDebug(" srv:", pthis.srvID, " conn", tcpConn.TcpConnectionID(), " from srv:", protoMsg.Id)
	if tcpConn != nil {
