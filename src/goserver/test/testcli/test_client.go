	...
	case net.Error:
		{
			if t.Timeout() {
				...
				if ok {
					...
					default:
						common.LogDebug("unknow net op err", st)
					}
				} else {
					common.LogDebug(" unkonw other :", t)
				}
			}
		}
/*	case *net.OpError:
	...
	default:
		common.LogDebug("other err:", t)
	}

	//lin_common.LogErr(err)
	if err == io.EOF{
		common.LogDebug("io eof")
		return false
	}
...
func (tcpInfo *ClientTcpInfo)GoClientTcpProcess() {
	...
	defer func() {
		err := recover()
		if err != nil {
			common.LogDebug("recover get err:", err)
		}
		common.LogDebug("call defer exit process coroutine")
		Global_wg.Done()
	}()
...
func (pthis *ClientTcpInfo)processSendMsgLoop(msg *interSendMsgLoop) {
	...
	for i := 0; i < msg.loopCount; i ++ {
		...
		if maxSeq < seq {
			common.LogDebug("~~~~~~err seq:", maxSeq)
		}
	}
}
...
func (tcpInfo *ClientTcpInfo)GoClientTcpRead(){
	...
	defer func() {
		...
		common.LogDebug("call defer exit read coroutine")
		Global_wg.Done()
	}()
	...
	for{
		...
		if !CheckError(err){
			...
			if con != nil {
				...
				common.LogDebug("connect to server err:", err)
			}
			continue
		}
		...
		for ; recvBuf.Len() >= PACK_HEAD_SIZE; {
			...
			case msgpacket.MSG_TYPE__MSG_LOGIN_RES:
				common.LogDebug(msgpacket.MSG_TYPE(curHead.packType), " proto msg:", protoMsg)
			default:
	...
	common.LogDebug("exit GoClientTcpRead", time.Now())
