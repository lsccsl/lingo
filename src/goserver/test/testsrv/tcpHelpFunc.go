...
func recvProtoMsg(tcpConn *net.TCPConn, timeOutSend int, retryCount int) (proto.Message, error) {
	...
	if readSize < PACK_HEAD_SIZE {
		...
		return nil, common.GenErr(common.ERR_NONE, " err:", errR.Error(), " read head size:", readSize)
	}
	...
	if packLen > 0 {
		...
		if readSize < packLen {
			...
			return nil, common.GenErr(common.ERR_NONE, " err:", errR.Error(), " read body size:", readSize)
		}
	} else {
		packLen = 0
	}
