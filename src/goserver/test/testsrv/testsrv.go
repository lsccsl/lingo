...
func (pthis*TestSrv)TestSrvDial() (err interface{}) {
	if pthis.tcpDial == nil {
		return common.GenErr(0, "no tcp dial conn")
	}
	defer func() {
		errR := recover()
		if errR != nil {
	...
	if err != nil {
		common.LogErr("write tcp err:", pthis.DialConnectionID, " srv:", pthis.srvId, " err:", err, " wz:", wz)
		return err
	}
	...
	for {
		...
		case *msgpacket.MSG_RPC_RES:
			if t.MsgId == msgRPC.MsgId {
				...
				common.LogDebug(" other id:", msgRPC.MsgId, " conn:", pthis.DialConnectionID, " srv:", pthis.srvId)
			}
		default:
			common.LogDebug(t, " err:", err, " conn:", pthis.DialConnectionID, " srv:", pthis.srvId)
		}
	}
...
func (pthis*TestSrv)tcpReDial() {
	...
	pthis.totalRedial ++
	if err != nil || conn == nil{
		common.LogDebug("dial err:", err, " connid:", pthis.DialConnectionID, " srv:", pthis.srvId, " conn:", conn)
		return
	} else {
		common.LogDebug("srv:", pthis.srvId, " addr:", pthis.addrRemote, " redial suc")
	}
...
/*			if pthis.tcpDial != nil {
			...
			common.LogDebug("rpc err:", err, " conn:", pthis.DialConnectionID, " srv:", pthis.srvId)
			if pthis.AutoRedial {
				pthis.tcpReDial()
			}
		}
...
func (pthis*TestSrv)TestSrvAcpt() (err interface{}) {
	if pthis.tcpAcpt == nil {
		return common.GenErr(0, "no tcp acpt conn")
	}
	defer func() {
		errR := recover()
		if errR != nil {
			common.LogDebug("acpt read:", errR)
			err = errR
		}
	}()
...
func (pthis*TestSrv)go_tcpAcpt() {
	...
	if pthis.srvId == 599 {
		common.LogDebug("srv:", pthis.srvId, " begin listen:", pthis.addrLocal)
	}
	...
	for{
		...
		if pthis.srvId == 599 {
			common.LogDebug("srv:", pthis.srvId, " begin accept")
		}

		conn, err := lsn.Accept()

		if pthis.srvId == 599 {
			common.LogDebug("srv:", pthis.srvId, " accept err:", err)
		}
...
/*		if pthis.tcpAcpt != nil {
		...
		pthis.totalReAcpt ++
		if err != nil || conn == nil{
			common.LogDebug("acpt err:", err, " conn:", pthis.AcptConnectionID, " srv:", pthis.srvId)
			continue
		}
		...
		go func(){
			...
			for {
				...
				if err != nil{
					common.LogDebug("acpt read err:", err, " conn:", pthis.AcptConnectionID, " srv:", pthis.srvId)
					break
				}
			}
