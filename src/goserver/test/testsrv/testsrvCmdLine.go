...
func MultiSrv(count int, idbase int, port_start int) {
	...
	for i := 0; i < count; i ++ {
		...
		port := port_start + i
		if srvid == 599 {
			common.LogDebug("srv:", srvid, " send http")
		}
...
func commandLineInit(){
	common.AddCmd("dump", "dump id", CommandDump)
	common.AddCmd("help", "help", common.DumpAllCmd)
	common.AddCmd("ms", "multi server", CommandMultiSrv)
	common.AddCmd("tr", "test rpc", CommandTestRPC)
	common.AddCmd("stopdial", "stop dial", CommandStopDial)
	common.AddCmd("autodial", "auto dial", CommandAutoDial)
}
