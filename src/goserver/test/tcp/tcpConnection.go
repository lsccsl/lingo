...
func startTcpConnection(connMgr InterfaceConnManage, conn net.Conn, closeExpireSec int) (*TcpConnection, error) {
	...
	if !ok {
		return nil, common.GenErr(common.ERR_not_tcp_connection)
	}
...
func startTcpDial(connMgr InterfaceConnManage, SrvID int64, ip string, port int,
	...
	ctx context.Context, rpcPool *common.CorPool) (*TcpConnection, error) {
	defer func() {
		err := recover()
		if err != nil {
			common.LogErr(err)
			...
			for i := 0; i < redialCount; i ++ {
				...
				if err != nil || conn == nil {
					...
					if strings.Index(err.Error(), "operation was canceled") >= 0 {
						common.LogDebug("srv:",  SrvID, " conn:", tcpConn.connectionID, " cancel dial", err)
						break DIAL_LOOP
					}
					//runtime.Gosched()
					if intervalMills <= 0 {
					...
					if conn != nil {
						common.LogDebug("srv:",  SrvID, " dial err:", err, " net conn:", conn)
						conn.Close()
					}
					time.Sleep(time.Millisecond * time.Duration(intervalMills + 1))
					continue
				} else {
					common.LogDebug("srv:", SrvID, " conn:", tcpConn.connectionID, " end dial suc", " connection id:", tcpConn.connectionID)
...
/*		} else {
		...
		for i:=0 ; i < redialCount; i ++ {
			...
			if err != nil {
				common.LogErr("will retry ", i, " ", redialCount, " ", err)
				continue
			}
		}
		if err != nil {
			common.LogErr("fail ", err)
			return nil, err
		}
...
/*	var TimerConnClose * time.Timer = nil
	...
	for {
		...
		if err != nil {
			common.LogDebug(pthis.connectionID, " tcp connection read err clientid:", pthis.ClientID, " srv:", pthis.SrvID,
	...
	for {
		...
		case tcpW := <- pthis.chMsgWrite:
			...
			if err != nil {
				...
				common.LogDebug(" tcp connection write err, conn:", pthis.TcpConnectionID(),
					" client id:", pthis.ClientID, " srv:", pthis.SrvID, " err:", err)
				pthis.netConn.Close()
				break WRITE_LOOP
			}
			if writeSZ < len(tcpW.bin) {
				common.LogErr("write sz wrong:", writeSZ)
			}
