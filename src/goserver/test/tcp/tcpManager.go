...
func (pthis *TcpMgr) CBGenConnectionID() TCP_CONNECTION_ID {
	return TCP_CONNECTION_ID(common.GenUUID64_V4())
}
...
func (pthis *TcpMgr)go_tcpAccept() {
	for {
		...
		if err != nil {
			common.LogErr("tcp accept err:", err)
			if conn != nil {
				conn.Close()
			}
			continue
		...
		if conn == nil {
			common.LogErr(" tcp conn is nil")
			continue
		}

		_, err = startTcpConnection(pthis, conn, pthis.closeExpireSec)
		if err != nil {
			common.LogErr("start accept tcp connect err", err)
		}
	}
...
func (pthis *TcpMgr) TcpMgrWait() {
	common.LogDebug("begin wait")
	pthis.wg.Wait()
	common.LogDebug("end wait")
}
