...
/**
* @brief 搜索最大前缀,如果存在 <"123", "12"> 输入"1234",则返回"123"对应的数据
*/
int StringSetSearchMaxPre(const HSTRING_SET hss, char * first, const char * last, unsigned long * data, size_t * data_size)
{
	string_tst_tree * tst_tree = (string_tst_tree *)hss;
	char_tst_node * current = NULL;
	char_tst_node * last_match = NULL;
	...
	if(last_match)
	{
		...
		if(INVAL_DATA == last_match->data)
			return -1;

		if(data)
			*data = last_match->data;

		if(data_size)
			*data_size = last_match->data_size;
